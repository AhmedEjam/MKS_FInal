...
object SettingsSanitizer {
    ...
    fun sortOption(value: String?): String = when (value?.uppercase()) {
        "TITLE", "LAST_EDIT", "LAST_STUDIED", "COMPLETION", "QUESTION_COUNT", "ACCURACY", "PROGRESS" -> value.uppercase()
        else -> "TITLE"
    }

    fun viewMode(value: String?): String = when (value?.uppercase()) {
        "GRID", "LIST" -> value.uppercase()
        else -> "LIST"
    }

    fun fontScale(value: Float?): Float = (value ?: 1.0f).coerceIn(0.85f, 1.35f)
    
    fun uiDensity(value: Float?): Float = (value ?: 1.0f).coerceIn(0.85f, 1.25f)
