...
class GenericImageExtractor {
    ...
    fun extractFromText(text: String?): ResolvedImage? {
        ...
        // 1. Data URL
        val dataUrlMatch = Regex("""(data:image/[a-zA-Z0-9.+-]+;base64,[A-Za-z0-9+/=\s]+)""", RegexOption.IGNORE_CASE)
            .find(raw)
        ...
        // 2. HTTP/HTTPS URL
        val urlMatch = Regex("""(https?://[^\s"'<>]+\.(?:png|jpe?g|gif|webp|svg|bmp|tiff?|avif|ico|pdf|png|jpg|jpeg)(?:[?#][^\s"'<>]*)|https?://[^\s"'<>]+)""", RegexOption.IGNORE_CASE)
            .find(raw)
        if (urlMatch != null) {
        ...
        if (imgMatch != null) {
            ...
            return if (src.startsWith("data:")) ResolvedImage(imageDataUrl = src, via = "html-img") else ResolvedImage(imageSource = src, via = "html-img")
        }
        ...
        if (mdMatch != null) {
            ...
            return if (src.startsWith("data:")) ResolvedImage(imageDataUrl = src, via = "markdown-img") else ResolvedImage(imageSource = src, via = "markdown-img")
        }
